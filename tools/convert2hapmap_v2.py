...
def convert_dart_to_hapmap(input_file,  output_file):
    ...
    #dart_snptable = pd.read_csv("DArTSNPs_v4.2.csv")
    ...
    for i, marker in enumerate(marker_names):
        try:
            ...
            # get the marker from the snp table dictionary
                ...
                alleles, chrom, pos = "NA/NA", "NA", "NA"
            finally:
                chrom, pos = val.split(',')[2], val.split(',')[3]
                alleles = val.split(',')[4]+ "/"+ (val.split(',')[5])
            #alt_snp_names_tokenized = marker.split("_")
            #if marker.startswith('chr'):
            #    chrom = alt_snp_names_tokenized[0]
            #    pos = alt_snp_names_tokenized[1]
            #elif marker.startswith('MSU'):
            #    chrom = alt_snp_names_tokenized[1]
            #    pos = alt_snp_names_tokenized[2]
            #    alleles = alt_snp_names_tokenized[3]
            #elif marker.startswith('IRGSP'):
            #    chrom = alt_snp_names_tokenized[1]
            #    pos = alt_snp_names_tokenized[2]
        except IndexError:
            alleles, chrom, pos = "NA/NA", "NA", "NA"  # if chromosome and position not specified

        
        # Get genotype calls for this marker
        genotypes = snp_data[marker].replace({".:.": "N", "-:-": "N"}).tolist()
